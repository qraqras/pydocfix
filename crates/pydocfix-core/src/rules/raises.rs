...
fn ris003(ctx: &RuleContext<'_>, diagnostics: &mut Vec<Diagnostic>) {
    for raise_entry in ctx.semantic.raises() {
        ris003_entry(ctx, diagnostics, raise_entry);
    }
}

fn ris003_entry(ctx: &RuleContext<'_>, diagnostics: &mut Vec<Diagnostic>, raise_entry: &Raise) {
