use std::path::PathBuf;

use pydocfix_core::{ClassDocstringStyle, TypeAnnotationStyle};

use crate::config::{OutputFormat, parse_class_docstring_style, parse_output_format, parse_type_annotation_style};

#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct CliArgs {
    pub(crate) debug_docstrings: bool,
    pub(crate) fix: bool,
    pub(crate) diff: bool,
    pub(crate) unsafe_fixes: bool,
    pub(crate) generate_baseline: bool,
    pub(crate) allow_optional_shorthand: bool,
    pub(crate) config_path: Option<PathBuf>,
    pub(crate) baseline_path: Option<PathBuf>,
    pub(crate) output_format: Option<OutputFormat>,
    pub(crate) type_annotation_style: Option<TypeAnnotationStyle>,
    pub(crate) class_docstring_style: Option<ClassDocstringStyle>,
    pub(crate) jobs: Option<usize>,
    pub(crate) select: Vec<String>,
    pub(crate) ignore: Vec<String>,
    pub(crate) paths: Vec<PathBuf>,
}

pub(crate) fn parse_cli_args(args: impl IntoIterator<Item = String>) -> Result<Option<CliArgs>, String> {
    let mut args = args.into_iter().collect::<Vec<_>>();
    if matches!(args.first().map(String::as_str), Some("check")) {
        args.remove(0);
    }

    if args.is_empty() || matches!(args.first().map(String::as_str), Some("-h" | "--help")) {
        return Ok(None);
    }

    let debug_docstrings = remove_flag(&mut args, "--debug-docstrings");
    let fix = remove_flag(&mut args, "--fix");
    let diff = remove_flag(&mut args, "--diff");
    let unsafe_fixes = remove_flag(&mut args, "--unsafe-fixes");
    let generate_baseline = remove_flag(&mut args, "--generate-baseline");
    let allow_optional_shorthand = remove_flag(&mut args, "--allow-optional-shorthand");
    let config_path = remove_option(&mut args, "--config")?.map(PathBuf::from);
    let baseline_path = remove_option(&mut args, "--baseline")?.map(PathBuf::from);
    let output_format = remove_option(&mut args, "--output-format")?
        .map(|value| parse_output_format(&value))
        .transpose()?;
    let type_annotation_style = remove_option(&mut args, "--type-annotation-style")?
        .map(|value| parse_type_annotation_style(&value))
        .transpose()?;
    let class_docstring_style = remove_option(&mut args, "--class-docstring-style")?
        .map(|value| parse_class_docstring_style(&value))
        .transpose()?;
    let jobs = remove_option(&mut args, "--jobs")?
        .map(|value| parse_jobs(&value))
        .transpose()?;
    let select = parse_rule_filter(remove_option(&mut args, "--select")?);
    let ignore = parse_rule_filter(remove_option(&mut args, "--ignore")?);

    if args.is_empty() {
        return Ok(None);
    }

    Ok(Some(CliArgs {
        debug_docstrings,
        fix,
        diff,
        unsafe_fixes,
        generate_baseline,
        allow_optional_shorthand,
        config_path,
        baseline_path,
        output_format,
        type_annotation_style,
        class_docstring_style,
        jobs,
        select,
        ignore,
        paths: args.into_iter().map(PathBuf::from).collect(),
    }))
}

pub(crate) fn print_help() {
    println!("pydocfix");
    println!();
    println!("Usage: pydocfix [check] [--debug-docstrings] <PATH>...");
    println!();
    println!("Checks Python docstrings with the Rust implementation.");
    println!();
    println!("Options:");
    println!("  --debug-docstrings");
    println!("  --fix");
    println!("  --diff");
    println!("  --unsafe-fixes");
    println!("  --config <PATH>");
    println!("  --baseline <PATH>");
    println!("  --generate-baseline");
    println!("  --select <CODE[,CODE...]>");
    println!("  --ignore <CODE[,CODE...]>");
    println!("  --output-format <full|concise>");
    println!("  --type-annotation-style <signature|docstring|both>");
    println!("  --class-docstring-style <class|init|both>");
    println!("  --allow-optional-shorthand");
    println!("  --jobs <N>");
}

fn parse_jobs(value: &str) -> Result<usize, String> {
    let jobs = value
        .parse::<usize>()
        .map_err(|_| format!("invalid --jobs value {value:?}; expected a positive integer"))?;
    if jobs == 0 {
        return Err("invalid --jobs value 0; expected a positive integer".to_string());
    }
    Ok(jobs)
}

fn remove_flag(args: &mut Vec<String>, flag: &str) -> bool {
    let original_len = args.len();
    args.retain(|arg| arg != flag);
    args.len() != original_len
}

fn remove_option(args: &mut Vec<String>, flag: &str) -> Result<Option<String>, String> {
    if let Some(index) = args.iter().position(|arg| arg == flag) {
        args.remove(index);
        if index >= args.len() {
            return Err(format!("missing value for {flag}"));
        }
        return Ok(Some(args.remove(index)));
    }

    let prefix = format!("{flag}=");
    if let Some(index) = args.iter().position(|arg| arg.starts_with(&prefix)) {
        let value = args.remove(index);
        return Ok(Some(value[prefix.len()..].to_string()));
    }

    Ok(None)
}

fn parse_rule_filter(value: Option<String>) -> Vec<String> {
    value
        .into_iter()
        .flat_map(|value| value.split(',').map(str::trim).map(str::to_string).collect::<Vec<_>>())
        .filter(|value| !value.is_empty())
        .collect()
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn returns_none_for_help() {
        assert!(parse_cli_args(["--help".to_string()]).unwrap().is_none());
    }

    #[test]
    fn parses_check_command_and_options() {
        let args = parse_cli_args([
            "check".to_string(),
            "--fix".to_string(),
            "--unsafe-fixes".to_string(),
            "--select".to_string(),
            "SUM,PRM001".to_string(),
            "--jobs=4".to_string(),
            "src".to_string(),
        ])
        .unwrap()
        .unwrap();

        assert!(args.fix);
        assert!(args.unsafe_fixes);
        assert_eq!(args.select, vec!["SUM", "PRM001"]);
        assert_eq!(args.jobs, Some(4));
        assert_eq!(args.paths, vec![PathBuf::from("src")]);
    }

    #[test]
    fn reports_missing_option_value() {
        let error = parse_cli_args(["--baseline".to_string()]).unwrap_err();

        assert_eq!(error, "missing value for --baseline");
    }

    #[test]
    fn reports_invalid_jobs() {
        let error = parse_cli_args(["--jobs=0".to_string(), "src".to_string()]).unwrap_err();

        assert_eq!(error, "invalid --jobs value 0; expected a positive integer");
    }
}
